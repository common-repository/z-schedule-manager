...
require( dirname(__FILE__) . '/storable_eva.php' );
...
abstract class _HC_ORM_EVA extends _HC_ORM
{
	protected $type = NULL;

	public function set_db( $db )
	{
		$this->storable = new _HC_ORM_Storable_Eva( 
			$db,
			$this->type
			);
	}
...
abstract class _HC_ORM extends _HC_MVC
{
	...
	{
		$return = $this->storable->count( $this->where );
		return $return;
	}
	...
	public function fetch_array( $fields = '*' )
	{
		...
		$return = $this->storable->fetch( $fields, $where, $this->limit, $order_by, $this->distinct );
...
// SAVE PROXY
	...
	protected function _update()
	{
		...
		if( $changes ){
			...
			$new_data = $this->prepare_for_save( $new_data );
